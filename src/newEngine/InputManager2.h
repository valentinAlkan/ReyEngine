...
#define EVENT_GENERATE_ID(UNIQUE_EVENT_ID) \
namespace UniqueEventIds{ static constexpr size_t EVENTID_##UNIQUE_EVENT_ID = UNIQUE_EVENT_ID;}

#define EVENT_GENERATE_STATIC_CONSTEXPR_EVENTID(UNIQUE_EVENT_ID) \
static constexpr EventId eventId = UniqueEventIds::EVENTID_##UNIQUE_EVENT_ID;

#define EVENT(EVENT_NAME, UNIQUE_EVENT_ID) \
   EVENT_GENERATE_ID(UNIQUE_EVENT_ID)                          \
   struct EVENT_NAME : public Event<EVENT_NAME, UniqueEventIds::EVENTID_##UNIQUE_EVENT_ID> { \
      EVENT_GENERATE_STATIC_CONSTEXPR_EVENTID(UNIQUE_EVENT_ID)    \
      explicit EVENT_NAME(const EventPublisher* publisher) : Event<EVENT_NAME, eventId>(publisher)
...
#define EVENT_ARGS(EVENT_NAME, UNIQUE_EVENT_ID, ...) \
   ...
   struct EVENT_NAME : public Event<EVENT_NAME, UniqueEventIds::EVENTID_##UNIQUE_EVENT_ID> { \
      EVENT_GENERATE_STATIC_CONSTEXPR_EVENTID(UNIQUE_EVENT_ID)    \
      explicit EVENT_NAME(const EventPublisher* publisher, __VA_ARGS__) : Event<EVENT_NAME, eventId>(publisher)


namespace UniqueEventIds{}
namespace ReyEngine{
   namespace Events {
      ...
      EVENT(InputEventKeyd, 23462346324){}
         InputInterface::KeyCode key;
         bool isDown = false;
         bool isRepeat = false;
      };
   ...
   //         switch (int(other.mouse.eventId)){
   //            case InputEventMouseMotion::eventId:
   //               new (&motion) InputEventMouseMotion(other.motion);
   //               break;
   //            case InputEventMouseButton::eventId:
   //               new (&button) InputEventMouseButton(other.button);
   //               break;
   //            case InputEventMouseWheel::eventId:
