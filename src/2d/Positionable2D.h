...
namespace ReyEngine{
   ...
   public:
      ...
      {}
      Pos<T> getPos() const {return transform.translation;}
      Pos<T> getCenter() const {return transform.translation + size / 2;}
      ...
      ReyEngine::Rect<T> getRect() const {return {transform.translation, size};}
      ...
      void* getAddr() const {return (void*)&transform.translation;}
//      std::shared_ptr<Collision::CollisionRect> make_collider(const std::string& instanceName);
   ...
   protected:
      ...
      void applyRect(const Rect<R_FLOAT>& rect){
         transform.translation = rect.pos();
         size = rect.size();
      }
