...
static constexpr inline Fraction easeInOutQuint(Fraction x) {
   return x.get() < 0.5 ? 16 * x.get() * x.get() * x.get() * x.get() * x.get() : 1 - std::pow(-2 * x.get() + 2, 5) / 2;
}
...
struct Easing {
   Easing(const std::chrono::steady_clock::time_point& startTime, std::chrono::milliseconds duration)
