...
namespace ReyEngine{
   ...
   std::shared_ptr<T> replace_child(N parent, InstanceName&& instanceName, Args&&... args){
      ...
      {
         if (auto oldChild = parent->removeChild(instanceName, true)) {
            index = oldChild.value()->getIndex();
         }
      }
   ...
   namespace Internal{
      struct ReyObject : public TreeStorable
      {
         ...
         //the (!is_convertible_v<N, string_view>) guard keeps this disjoint from the implicit-parent
         // overload above - without it, a lone string argument would be ambiguous between "this is
         ...
         void defer(std::function<void()>&& func, Args&&... args) {_node->defer(std::move(func), std::forward<Args>(args)...);}
      protected:
         ReyObject(){
